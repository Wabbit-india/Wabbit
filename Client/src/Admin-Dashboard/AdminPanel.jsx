...
import { MdAccountBox, MdWork, MdInsertDriveFile } from 'react-icons/md'; // Icons

function AdminPanel() {
  const [activeProfile, setActiveProfile] = useState("SignUp");
  const [activeId, setActiveId] = useState(1); // for highlighting selected button
  ...
  const changeHandler = (id, name) => {
    setActiveId(id);
    setActiveProfile(name); // switch component view
  };
  ...
  return (
    <div>
      <div className="lg:w-[100vw] bg-bgmain py-14 h-screen flex flex-col lg:justify-center sm:flex-row">
        ...
        {/* Sidebar */}
<aside className="w-full relative left-[-100px] lg:w-[300px] h-[100%] bg-white shadow-lg px-4 py-6 flex flex-col gap-4 items-start">
  {sidebutton.map((item) => (
    <div
      key={item.id}
      ...
      className={`cursor-pointer flex items-center gap-3 w-full px-4 py-2 rounded-md transition-all ${
        activeId === item.id ? 'bg-green-400 text-white' : 'hover:bg-green-100 text-gray-800'
      }`}
    >
